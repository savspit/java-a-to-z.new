...
public class JobOffersParsing {
    ...
    private JobOffersStorage dbWorking = new JobOffersStorage();
    ...
    public void getJobOffers() {
        if (this.dbWorking.isFirstRun()) {
            getJobOffersFromStartDate(getCurrentDateMinusYear());
        } else {
            getJobOffersFromStartDate(this.dbWorking.getLastRunTime());
        }
    }
                ...
                for (Element elem : allElements) {
                    ...
                    if (isClosedPost.size() == 0) {
                        ...
                        if (jobOffer.hasClass("") && isJavaJobOffer(jobOffer) ) {
                            ...
                            dbWorking.addDataInDB(offerLink, offerText, offerAuthor, offerAuthorLink, offerDate);
                        }
                    }
                }
            } catch (IOException e) {
    ...
    public String getNextMainJobOffersUrl(int offersPageCount) {
        return String.format("%s/%s", this.dbWorking.getOffersUrl(), offersPageCount);
    }
